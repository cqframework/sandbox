...
import AppShell from "../AppShell/app-shell";
...
export class MainView extends Component {
    ...
    async componentDidMount() {
        ...
        if (!validHooks.includes(parsedHook)) parsedHook = null;

        // Derive screen/hook from PATH (e.g., "/rx-view") if present
        const pathSeg = (window.location && window.location.pathname || '/').replace(/^\//, '') || null;
        const pathHook = pathSeg === 'rx-view' ? 'order-select' : pathSeg === 'rx-sign' ? 'order-sign' : pathSeg;

        // Choose in order of precedence: query → path → persisted → default
        const nextScreen = parsedScreen || pathSeg || localStorage.getItem('PERSISTED_screen') || 'patient-view';
        const nextHook = parsedHook || (pathSeg ? pathHook : null) || localStorage.getItem('PERSISTED_hook') || 'patient-view';

        // Set the hook in context
        this.props.setHook(nextHook, nextScreen);
    ...
    componentDidUpdate(prevProps) {
        // When the selected screen changes, reflect it into the URL as /<screen>?screen=<screen>&hook=<hook>
        if (this.props.screen !== prevProps.screen) {
            const screen = this.props.screen || 'patient-view';
            const hook =
                screen === 'rx-view' ? 'order-select' :
                    screen === 'rx-sign' ? 'order-sign'   :
                        screen;
    ...
    render() {
        ...
        const isCardDemo = this.props.screen === 'card-demo';
        const container = !isCardDemo ? (
        ...
        return (
            <AppShell
                activeSegment={this.props.screen}
                onNavigate={(segment) => {
                    const hook =
                        segment === 'rx-view' ? 'order-select' :
                            segment === 'rx-sign' ? 'order-sign'   :
                                segment;
                    this.props.setHook(hook, segment);
                }}
            >
                <Backdrop
